...
public class Obstacles {
			...
			for(int j=0; j<obstacle.size(); j++){
				if(i != j)
				{
					if(obstacle.elementAt(i) != null){
						...
						path.moveTo(obstacle.elementAt(i).getX(), obstacle.elementAt(i).getY());
						path.lineTo(obstacle.elementAt(j).getX(), obstacle.elementAt(j).getY());
						//check = check || obSeg.intersectsLine(path);
						check = check || obSeg.op(path, Path.Op.INTERSECT);
					}
					else
						break;
				}
	 ...
	 * created to adapt the code to Android
	 * @param x,y,x1,y1,x2,y2 - point(x,y) line(x1y1,x2y2)
	 * @return double - value of distance
	 */
	private double distPtSeg(double x, double y, double x1, double y1, double x2, double y2){
		Double A = x - x1;
		Double B = y - y1;
		Double C = x2 - x1;
		Double D = y2 - y1;
	...
	public boolean validItemPos(ItemPosition destination, double radius){
		...
		for(int j = 0; j < obstacle.size() ; j++){
			...
			if(distPtSeg(destination.x, destination.y, curpoint.x, curpoint.y, nextpoint.x, nextpoint.y) < radius){
				return false;
			}
...
//			double dist4 = current.ptSegDist(segment.x2, segment.y2);
			double dist1 = distPtSeg(destNode.position.x,destNode.position.y, curpoint.x,curpoint.y,nextpoint.x,nextpoint.y);
			double dist2 = distPtSeg(currentNode.position.x,currentNode.position.y, curpoint.x,curpoint.y,nextpoint.x,nextpoint.y);
			double dist3 = distPtSeg(curpoint.x,curpoint.y, destNode.position.x,destNode.position.y,currentNode.position.x,currentNode.position.y);
			double dist4 = distPtSeg(nextpoint.x,nextpoint.y, destNode.position.x,destNode.position.y,currentNode.position.x,currentNode.position.y);
